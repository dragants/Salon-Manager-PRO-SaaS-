...
/**
 * Ista JWT / token_version / pretplata logika kao auth.middleware,
 * ali token može i kao ?token= (potrebno za EventSource).
 */
module.exports = async function sseAuthMiddleware(req, res, next) {
  if (!JWT_SECRET) {
  ...
  try {
    ...
    const jwtTv =
      decoded.tv !== undefined && decoded.tv !== null ? decoded.tv : 0;
    if (dbTv !== jwtTv) {
